...
namespace LoggingKata
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (lines.Length == 0)
            {
                logger.LogError("No line showing");
            }
            if (lines.Length == 1)
            {
                logger.LogWarning("It has one line");
            }
            ...
            Console.WriteLine($"{tacoBellA.Name} and {tacoBellB.Name} the distance are farthest from one and other");
